"""
Code based almost entirely from [allennlp]
"""
...
def _choice(
    ...
    def get_buffer() -> np.ndarray:
        ...
        _samples = np.exp(log_samples).astype("int64") - 1
        return np.clip(_samples, a_min=0, a_max=num_words - 1)
...
class SampledSoftmaxLoss(torch.nn.Module):
    ...
    !!! NOTE
        In all cases the weights are dimensioned as num_words and do not include an entry for the padding (0) id.
    ...
    def _forward_train(
        ...
        # and the remaining ones are the negative samples.
        ...
        true_logits = (
                ...
                + true_b
                - torch.log(target_expected_count + tiny_value_of_dtype(target_expected_count.dtype))
        )
        # [batch_size, n_samples]
        sampled_logits = (
                torch.matmul(embeddings, sampled_w.t())
                + sampled_b
                - torch.log(sampled_expected_count + tiny_value_of_dtype(sampled_expected_count.dtype))
        )
    ...
    def _forward_eval(self, embeddings: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.nll_loss(log_softmax, targets.long(), reduction="sum")
